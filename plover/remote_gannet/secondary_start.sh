...
sudo LD_LIBRARY_PATH=$VMFT_ROOT/rdma-paxos/target:$LD_LIBRARY_PATH x86_64-softmmu/qemu-system-x86_64 -boot c -m 2048 -smp 2 \
-qmp stdio -vnc :7 -name secondary -enable-kvm \
...
-drive if=virtio,id=active-disk0,driver=replication,mode=secondary,file.driver=qcow2,top-id=active-disk0,file.file.filename=/home/hkucs/active_disk.img,file.backing=colo-disk0\
-drive if=none,id=hidden-disk0,driver=replication,mode=secondary,file.driver=qcow2,top-id=hidden-disk0,file.file.filename=/home/hkucs/hidden_disk.img,file.backing=colo-disk0\
