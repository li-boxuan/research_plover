...
if [ "$set_secondary" = true ] ; then
    ssh hkucs@202.45.128.168 "\
    (\
    echo {\'execute\':\'qmp_capabilities\'};\
    echo {\'execute\': \'nbd-server-start\', \'arguments\': {\'addr\': {\'type\': \'inet\', \'data\': {\'host\': \'10.22.1.9\', \'port\': \'8889\'} } }};\
    echo {\'execute\': \'nbd-server-add\', \'arguments\': {\'device\': \'hidden-disk0\', \'writable\': true } };\
    sleep 10;\
...
if [ "$set_primary" = true ] ; then
    ...
    #debug mode
    echo "{ 'execute': 'migrate-set-parameters' , 'arguments': { 'x-checkpoint-delay': 100 } }"
    sleep 10;
    ) | telnet localhost 4444
    sleep 30
fi
