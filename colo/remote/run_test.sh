...
if [ "$set_secondary" = true ] ; then
    (
        nbd_server_start 10.22.1.5:8889
...
if [ "$set_primary" = true ] ; then
    sleep 10;
    (
        child_add disk1 child.driver=replication,child.mode=primary,child.file.host=10.22.1.5,child.file.port=8889,child.file.export=colo1,child.file.driver=nbd,child.ignore-errors=on
        migrate_set_capability colo on
        migrate tcp:10.22.1.5:8888
